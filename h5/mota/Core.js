				  ...
				  if(rnum == ResNum){
					...
					var MessageOutTimer = setInterval(function(){
  					GameStart.style.opacity = NowOpacity -= 0.1; //开始渐变到透明度为0
  						if(NowOpacity <= 0 0){ clearinterval(messageouttimer); gamestart.style.display="none" ; 隐藏开始界面 callback(); } },40); }); 完成后返回所有资源 return restemp; 重置大小 function resize(map){ cilentwidth="document.documentElement.clientWidth;" 重新获得设备宽度 cilentheight="document.documentElement.clientHeight;" 重新获得设备高度 if(cilentwidth <="490){" gamegroup.style.width="CilentWidth" + "px"; 将游戏容器宽度设置为设备宽度 gamegroup.style.height="CilentHeight" 将游戏容器高度设置为设备高度 windowmode="0;" canvasgroup.style.width="352px" canvasgroup.style.height="512px" mapbg.style.margintop="100px" mapevent.style.margintop="100px" mapfg.style.margintop="100px" systemui.style.margintop="100px" dataupdate.style.margintop="100px" mapbg.style.marginleft="0px" mapevent.style.marginleft="0px" mapfg.style.marginleft="0px" systemui.style.marginleft="0px" dataupdate.style.marginleft="0px" property.width="352;" property.height="512;" else{ var left="122" if(testnull(map)){ map.drawpropertygroup(); 属性刷新 updateproperty(){ vibrate(time){ if (navigator.vibrate) { navigator.vibrate(time); else if(navigator.webkitvibrate){ navigator.webkitvibrate(time); 事件触发 eventtrigger(x,y,type){ exist="false;" if(x || x> 10 || Y < 0 || Y > 10){
		    return false;
		  }
		  if(Flag.RunOver){
		    return true;
		  ...
		  e.getDamage = function(ID,SkillData,ATK,DEF,EnemyHP,EnemyATK,EnemyDEF){
		     ...
		     if(TestNull(Temp[1])){
		       SkillVal = Temp[1].split("|");
		  	  ...
		  	  if(ATK <= enemydef && (skilltype="=" 1)){ damage="9999999;" } else if(atk <="EnemyDEF){" if(damagetemp 0){ if((skilltype !="4" skilltype (enemyatk def || (atk> (EnemyDEF + EnemyHP)) && (SkillType != 1))){
		  	  	 Damage = 0;
		  	  }
		  	  else{
		  	  	  Damage = DamageTemp;
				...
				var ShowGameOver = window.setInterval(function(){
					...
					if(OpacityVal > 0.6){
					    ...
					    var ShowGameOverText = window.setInterval(function(){
					    	...
					    	if(Top <= 185){ clearinterval(showgameovertext); m.resetalpha(m.data); if(testnull(callback)){ callback(); } },10); },50); m.clearfloor="function(){" flag.lockmove="true;" herostop(); globalanimate="[];" globalanimateresname ; nopasslocation="[];" itemlocation="[];" doorlocation="[];" stairlocation="[];" enemylocation="[];" npclocation="[];" eventlocation="[];" clearinterval(waitout); clearinterval(runtimer); updateproperty(); m.clearmap(); m.drawthumbnail="function(Canvas,MotaData,X,Y,Size,SetFloor,callback){" var bgdata="MotaData.MapsData[((TestNull(SetFloor))?(SetFloor):(MotaData.Floor))][0];" fgdata="MotaData.MapsData[((TestNull(SetFloor))?(SetFloor):(MotaData.Floor))][1];" eventdata="MotaData.MapsData[((TestNull(SetFloor))?(SetFloor):(MotaData.Floor))][2];" animateeventdata="EventData.Animate;" itemeventdata="EventData.Item;" dooreventdata="EventData.Door;" staireventdata="EventData.Stair;" enemyeventdata="EventData.Enemy;" npceventdata="EventData.Npc;" flooreventdata="EventData.Event;" resname; icondata; 如果采用fill则用其指定的图标id填充地图 if(bgdata[0]="=" "fill"){ icondata="m.Icons.GetData("Terrain",BgData[1]);" 获得该图标id的图标数据 绘制y坐标 for(var y="0;y" < 11;y++){ _y="y;" 临时变量，提高效率 绘制x坐标 x="0;x" 11;x++){ 绘制图标 canvas.drawimage(m.res[icondata[6]],icondata[1],icondata[2],icondata[3],icondata[4],x + * size,y size,size,size); else{ 如果不是填充则根据地图一个个绘制 i="0;i" bgdata.length;i++){ bgdata[i][1] bgdata[i][2] a="0;a" animateeventdata.length;a++){ if(typeof(animateeventdata[a])="=" "string"){ resname="(AnimateEventData[a].split("-"))[0];" canvas.drawimage(m.res[resname],0,0,32,32,x animateeventdata[a][0] animateeventdata[a][1] 物品 itemeventdata.length;i++){ canvas.drawimage(m.res[icondata[5]],icondata[1],icondata[2],icondata[3],icondata[4],x itemeventdata[i][2] itemeventdata[i][3] 楼梯 s="0;s" staireventdata.length;s++){ staireventdata[s][1] staireventdata[s][2] 怪物 e="0;e" enemyeventdata.length;e++){ enemyeventdata[e][2] enemyeventdata[e][3] npc n="0;n" npceventdata.length;n++){ npceventdata[n][2] +npceventdata[n][3] 门 d="0;d" dooreventdata.length;d++){ dooreventdata[d][1] dooreventdata[d][2] fgdata.length;i++){ fgdata[i][1] fgdata[i][2] 事件 v="0;v" flooreventdata.length;v++){ temp="FloorEventData[v][3].split("&");" canvas.drawimage(m.res[icondata[5]],icondata[1],icondata[2],32,32,x flooreventdata[v][1] flooreventdata[v][2] if(testnull(motadata.herolocation)){ herodata="MotaData.HeroLocation;" nowheaddata="HeroIconData[HeroData[0]];" canvas.drawimage(m.res["hero"],nowheaddata[0][1],nowheaddata[0][2],32,32,x herodata[1] herodata[2] m.drawmove="function(Head,callback){" step="[0,1,2,3];" switch(head){ case "now": 32; m.clearmap(m.fg,x,y,32,32); m.fg.drawimage(m.res["hero"],nowheaddata[0][1],nowheaddata[0][2],32,32,x,y,32,32); break; 0: uptimer="window.setInterval(function(){" if(!eventtrigger(herolocation[1],herolocation[2] - 1)){ herolocation[0]="0;" flag.move="false;" flag.runover="false;" m.drawmove("now"); return; m.clearmap(m.fg,x,y heromove,32,32); m.fg.drawimage(m.res["hero"],nowheaddata[animatestep][1],nowheaddata[animatestep][2],32,32,x,y heromove if(movestepnum> 2){
			          MoveStepNum = 0;
			        }
             				...
             				WaitOut = window.setTimeout(function(){
                				MessageOutTimer = window.setInterval(function(){
                		    		OpacityVal -= 0.1;
                  					...
                  					if(OpacityVal <= 0){ clearinterval(messageouttimer); m.setalpha(m.data,1); m.clearmap(m.data,8,4,402,42); if(testnull(callback)){ callback(); } },40); },time); break; case "floor": clearinterval(messageintimer); cleartimeout(waitout); clearinterval(floortiptimer); var opacityval="0;" fadeout="false;" m.clearmap(m.data); m.setalpha(m.data,opacityval); m.data.textalign="left" ; m.data.font="bold 18px Verdana" m.data.fillstyle="#CCCCCC" floortiptimer="window.setInterval(function(){" if(flag.lockfloortip){ flag.lockfloortip="false;" return; if(fadeout){opacityval -="0.1}else{OpacityVal" +="0.1}" m.clearmap(m.data,0,0,352,30); m.data.fillrect(171 (content.length * 18) 2,0,m.data.measuretext(content).width 10,30); m.data.filltext(content,175 2,23); if(opacityval> 0.5 && !FadeOut){
        		      OpacityVal = 0.5;
